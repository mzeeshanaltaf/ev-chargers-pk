...
function validateContact(data: { name: string; email: string; description: string }): ContactErrors {
  ...
  if (!data.description.trim()) {
    ...
    errors.description = "Message must be under 2000 characters";
  }
      ...
      <main className="max-w-xl mx-auto px-6 py-12">
        ...
        {isSubmitted ? (
          <div className="rounded-xl border border-brand/30 bg-brand/5 p-8 text-center">
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-brand text-white font-medium text-sm hover:brightness-110 transition-all"
            >
              Back to Map
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                rows={5}
                maxLength={2000}
                className={`w-full px-3 py-2 rounded-lg bg-surface border border-border text-text-primary placeholder:text-text-secondary/50 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-sm resize-none transition-all ${
